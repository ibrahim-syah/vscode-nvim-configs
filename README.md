...
- Configure neovim binary path in the extension setting if necessary (default is fine)
- ensure that nevoim works in vscode
- install copy and paste the init.lua and lua/core/init.lua to nvim config folder (usually in .config/nvim
- install all plugin (leap) by just opening up vscode (packer is only loaded in nvim if in vscode mode) or entering :PackerInstall or :PackerSync
- you are good to go :)
